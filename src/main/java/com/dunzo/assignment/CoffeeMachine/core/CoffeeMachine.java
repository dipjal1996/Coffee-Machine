...
import static com.dunzo.assignment.CoffeeMachine.exception.ErrorEnum.*;

/**
 * Used the observer design pattern here, where {@link CoffeeMachine} is a subject and outlets is the list of Observers to this subject, where {@link Outlet} is the Observer.
 * Whenever, a new beverage request comes up, it has three fields - outletNumber, beverage and time
 * Now, CoffeeMachine forwards the beverage request to the respective outlet to prepare it.
 * If the outletNumber is invalid, it throws {@link CoffeeMachineException} error.
 * If the beverage is not supported by the machine, it throws {@link CoffeeMachineException} error.
 */
public class CoffeeMachine {

    //list of outlets (Observers) attached to this coffee machine
    private List<Outlet> outlets;

    //map that stores the current quantity of each ingredients in the machine
    private Map<String, Double> ingredientsQuantity;

    //map that stores the ingredients required for each beverage supported by this machine
    ...
    public CoffeeMachine(int numOutlets, Map<String, Double> ingredientsQuantity, Map<String, Map<String, Double>> beverageInfo) {
    ...
    /**
     * Takes in the prepare beverage request, forwards the request to the respective outlet.
     * Returns the string which indicates whether the beverage have been prepared or could not be prepared with some reasons.
    ...
    public String prepareBeverage(BeverageRequest beverageRequest) throws CoffeeMachineException {
        ...
        OutletResponse outletResponse = servingOutlet.prepareBeverage(beverageRequest.getBeverage(), beverageRequest.getTime());
        ...
        return String.format("The requested beverage %s could not be prepared because the quantity of below ingredients are insufficient, please refill these ingredients and try again.\n%s",
